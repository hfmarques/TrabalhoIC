...
void Graph::setGraphLabel(std::string label) {
   this->label = label;
}
void Graph::setVertexLabelToId(unsigned int id) {
   setVertexLabel(id, id);
}
...
void ProblemVertexData::setAsDepot(){
   depot = true;
}

bool ProblemVertexData::isDepot(){
   return depot;
}
